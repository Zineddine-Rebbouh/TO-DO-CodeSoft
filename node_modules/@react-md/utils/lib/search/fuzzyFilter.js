...
var getFuzzyRegExp_1 = require("./getFuzzyRegExp");
...
function fuzzyFilter(query, searchable, options) {
    ...
    var _a = (0, defaults_1.defaults)(options, utils_1.DEFAULT_SEARCH_OPTIONS), getItemValue = _a.getItemValue, valueKey = _a.valueKey, trim = _a.trim, ignoreWhitespace = _a.ignoreWhitespace;
    query = (0, utils_1.getSearchString)(query, false, trim, ignoreWhitespace);
    if (!searchable.length || !query) {
        return searchable;
    }
    var queryRegExp = (0, getFuzzyRegExp_1.getFuzzyRegExp)(query);
    return searchable.filter(function (item) {
        var value = (0, utils_1.getSearchString)(getItemValue(item, valueKey), false, trim, ignoreWhitespace);
